...
`qiuzhao` 是一套面向中文校招与实习申请的开源求职系统。它把岗位搜索、匹配判断、简历修改与证据审计、网申和进度管理、面试准备、开源贡献与日常学习，组织成一条可以执行、验证与复盘的工作流。

它包含两个彼此独立、又可以组合使用的部分：

| 入口 | 适合场景 | 你会得到什么 |
|---|---|---|
| [求职 Skills](skills/) | 已经有 JD、简历或网申页面，希望获得针对性协助 | 岗位排名、简历与审计、投递记录、面试题包、开源贡献 |
...
它也维护投递、筛选、测评、面试、Offer、拒绝和下一步行动的证据化历史。
...
### 5. 简历证据审计 `resume-auditor`

把简历拆成可独立核验的主张，检查角色边界、个人归因、时间线、指标口径和强措辞，给出可直接替换的修正版与待补证据。

### 6. 开源贡献 `oss-contributor`

寻找符合目标岗位且遵守上游规则的贡献机会，先准备和验证本地 diff，再逐项确认 fork、push 或 PR 等外部操作。只有合并后的真实范围才能进入简历证据台账。
...
3. 使用 $resume-auditor 检查角色、指标、时间线和证据缺口。
4. 收到网申后，使用 $application-form-helper 检查字段冲突并维护投递进度。
5. 面试前使用 $interview-prep 生成项目深挖与技术追问，并把缺口加入六周冲刺计划。
6. 使用 $oss-contributor 准备岗位相关的真实开源贡献，合并后再回写简历证据。
```
...
[MIT](LICENSE)。整合设计借鉴的第三方项目及其许可证见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
